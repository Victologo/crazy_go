...
call npx tsx ml_training\generate_games.ts --games 1000 --size 9 --batch-size 100 --output ml_training\data\phase_new
if errorlevel 1 goto error_gen

echo.
echo ============================================================
echo  STEP 2 - Training CrazyGoNet on GPU (100,000 steps)
echo  Starting from SCRATCH with double capacity!
echo ============================================================
echo.

python ml_training\train.py --data ml_training\data\phase_new --checkpoint-dir ml_training\checkpoints --max-steps 100000 --batch-size 128
