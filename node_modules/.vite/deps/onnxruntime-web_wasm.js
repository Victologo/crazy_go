...
async function Vr(n = {}) {
	...
	var g, b, y, T, I, _, $ = console.log.bind(console), v = console.error.bind(console), O = $, N = v, D = !1, k = (e) => e.startsWith("file://");
	function w() {
	...
	var H, ne, pe, B, W, re, me, A, K, je = !1;
	function ee() {
		var e = ge.buffer;
	...
	function j(e) {
		throw N(e = "Aborted(" + e + ")"), D = !0, e = new WebAssembly.RuntimeError(e + ". Build with -sASSERTIONS for more info."), I?.(e), e;
	}
	function q() {
		return { a: {
			T: fa,
	...
	function Io(e) {
		var r = new xt(e >>>= 0);
		return (w(), H)[r.Qb + 12 >>> 0] == 0 && (En(r, !0), nt--), Tn(r, !1), tt.push(r), fr(e);
	}
	var Fe = 0, Bo = () => {
		C(0, 0);
		var e = tt.pop();
	...
	var Po = () => {
		...
		var r = e.Ub;
		throw (w(), H)[e.Qb + 13 >>> 0] == 0 && (tt.push(e), Tn(e, !0), En(e, !1), nt++), Vt(r), Fe = r;
	};
	function Uo(e, r, i) {
		var s = new xt(e >>>= 0);
...
`))[0] == "Error" && s.shift(), jn(s);
	...
	function it() {
		if (0 < Re) te = it;
			...
			0 < Re ? te = it : (t.calledRun = !0, D || (he(), T?.(t)));
		}
	}
...
var xe = F(() => {
	...
	un = !1, St = !1, no = !1, fs = () => {
		...
		if (t > 0 && I.push(new Promise((_) => {
			...
			y(v).then((O) => {
				St = !1, un = !0, sn = O, _(), b && URL.revokeObjectURL(b);
			}, (O) => {
				St = !1, no = !0, $(O);
			});
	...
	ws = (n, t) => {
		...
		try {
			...
			for (let D of t?.executionProviders ?? []) if ((typeof D == "string" ? D : D.name) === "webnn") {
				if (o.shouldTransferToMLTensor = !1, typeof D != "string") {
					let w = D, Z = w?.context, H = w?.gpuDevice, ne = w?.deviceType, pe = w?.powerPreference;
					Z ? o.currentContext = Z : H ? o.currentContext = await o.webnnCreateMLContext(H) : o.currentContext = await o.webnnCreateMLContext({
						deviceType: ne,
						powerPreference: pe
					});
			...
			let [b, y] = bs(d), T = !!t?.enableGraphCapture, I = [], _ = [], $ = [], v = [];
			for (let D = 0; D < b; D++) {
				let [k, w, Z] = co(d, D);
		...
		m[5];
		let _ = t.length, $ = u.length, v = 0, O = [], N = [], D = [], k = [], w = [], Z = c.stackSave(), H = c.stackAlloc(_ * l), ne = c.stackAlloc(_ * l), pe = c.stackAlloc($ * l), B = c.stackAlloc($ * l);
		try {
			...
			for (let A = 0; A < $; A++) {
				...
				let je = c.stackSave(), ee = c.stackAlloc(4 * l), he = !1, j, q = 0;
				try {
					...
					if (j === "string") {
						...
						he = !0, re.push([
							...
							{
								mlTensor: ye,
								download: c.webnnCreateMLTensorDownloader(q, j),
								dispose: () => {
					...
					c.stackRestore(je), j === "string" && q && c._free(q), he || c._OrtReleaseTensor(K);
				}
			}
...
//#endregion
export { ts as InferenceSession, xr as TRACE, Pe as TRACE_EVENT_BEGIN, Ue as TRACE_EVENT_END, _e as TRACE_FUNC_BEGIN, De as TRACE_FUNC_END, de as Tensor, au as default, Y as env, qe as registerBackend };
